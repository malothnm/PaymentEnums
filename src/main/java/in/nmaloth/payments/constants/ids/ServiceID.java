package in.nmaloth.payments.constants.ids;

public class ServiceID {

    public static final String CARD_LIMIT_VALIDATOR = "CARD_LIMIT_VALIDATOR";
    public static final String CARD_VALIDATOR = "CARD_VALIDATOR";
    public static final String ACCOUNT_VALIDATOR = "ACCOUNT_VALIDATOR";
    public static final String CUSTOMER_VALIDATOR = "CUSTOMER_VALIDATOR";
    public static final String CURRENCY_CONVERSION = "CURRENCY_CONVERSION";
    public static final String MESSAGE_DISTRIBUTOR = "MESSAGE_DISTRIBUTOR";
    public static final String ADDRESS_VERIFIER = "ADDRESS_VERIFIER";
//    public static final String CRYPTO_SERVICE = "CRYPTO_SERVICE";
    public static final String CVC_SERVICE= "CVC_SERVICE";
    public static final String CVC2_SERVICE = "CVC2_SERVICE";
    public static final String CAVV_SERVICE = "CAVV_SERVICE";
    public static final String PIN_SERVICE = "PIN_SERVICE";
    public static final String ARQC_SERVICE = "ARQC_SERVICE";
    public static final String SCRIPT_GENERATION_SERVICE = "SCRIPT";
    public static final String AGGREGATOR = "AGGREGATOR";
    public static final String PARAMETER_VALIDATOR = "PARAMETER_VALIDATOR";
    public static final String NO_VALIDATION = "NO_VALIDATION";




}
